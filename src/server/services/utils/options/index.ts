...
interface OptionsParsers { [ key: string ]: (v: unknown) => number | never }

const optionParsers: OptionsParsers = {
  skip: function (v: unknown) {
    if (v === undefined) return 0
    const skip = parseInt(v as string)
    if (!Number.isSafeInteger(skip)) throw UNVALID_PAYLOAD_DATA_ERROR.getWithCustomMessage('Skip option must be a parsable safe integer')
    return skip
  },
  limit: function (v: unknown): number | never | 10 {
    if (v === undefined) return 0
    const limit = parseInt(v as string)
    if (!Number.isSafeInteger(limit)) throw UNVALID_PAYLOAD_DATA_ERROR.getWithCustomMessage('Limit option must be a parsable safe integer')
